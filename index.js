...
app.get('/items', async (req, res) => {
  try {
    ...
    const priceRes = await fetch(`https://west.albion-online-data.com/api/v2/stats/prices/${ids}?locations=Caerleon,Bridgewatch,Lymhurst,Martlock,Thetford,Fort Sterling,Brecilien`);
    const prices = await priceRes.json();

    console.log('✅ Datos obtenidos desde Albion2D:', prices.length);
    res.json(prices);
  } catch (error) {
    console.error('❌ Error en backend1:', error.message);
    res.status(500).json({ error: 'Error interno en backend1' });
  }
});

app.listen(PORT, () => {
  console.log(`✅ Backend1 corriendo en puerto ${PORT}`);
});
